...
*   **SSH Proxy (Dynamic Forwarding)**: `ssh://user@host:22?key=...` (key/ssh-agent) or `ssh://user:pass@host:22` (Windows + bundled plink / PuTTY). The app spawns a local SOCKS5 and routes traffic through it. If you want fully non-interactive plink, set `?hostkey=...` (plink `-hostkey`).
